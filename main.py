...
- 堆和栈
  - 为了防止上下文过长，模拟计算机的调用栈来管理对话上下文。任何frame内的上下文都会随着frame的弹出而被清除。这个frame非常类似于函数调用栈中的函数调用帧。如果某个子任务可能会产生很长的上下文（例如文件加载、浏览网页、或者多轮对话），就像调用一个函数一样，创建一个新的frame并push到栈顶。frame的goal是这个frame的子目标，expected_outcome是这个frame被pop时应该返回的内容。在子任务达成后，弹出这个frame并返回expected_outcome。否则，继续在当前frame内对话。
